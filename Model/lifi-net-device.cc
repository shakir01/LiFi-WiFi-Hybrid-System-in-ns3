...
LiFiNetDevice::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LiFiNetDevice")
    ...
    .AddAttribute ("Mtu", "The MAC-level Maximum Transmission Unit",
                   ...
                   MakePointerChecker<LiFiChannel> ()).AddAttribute ("Mac", "The MAC layer attached to this device.",
                   PointerValue (),
                   MakePointerAccessor (&LiFiNetDevice::GetMac,
                                        &LiFiNetDevice::SetMac),
                   MakePointerChecker<TdmaMac> ()).AddAttribute ("TdmaController", "The tdma controller attached to this device",
                   PointerValue (),
                   MakePointerAccessor (&LiFiNetDevice::GetTdmaController,
                                        &LiFiNetDevice::SetTdmaController),
...
void LiFiNetDevice::SetTrafficType(uint8_t trafficType)
{
  m_trafficType = trafficType;
}

uint8_t LiFiNetDevice::GetTrafficType()
{
  return m_trafficType;
}

Ptr<TdmaController>  LiFiNetDevice::GetMacHigh (void) const
{
  return 0;
}
...
void LiFiNetDevice::SetMac (Ptr<TdmaMac> mac)
{
  m_mac = mac;
...
Ptr<TdmaMac> LiFiNetDevice::GetMac (void) const
{
  return m_mac;
}

void
LiFiNetDevice::SetTdmaController (Ptr<TdmaController> controller)
{
...
Ptr<TdmaController>
LiFiNetDevice::GetTdmaController (void) const
{
...
//std::cout<<"LiFiNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber);"<<" node id "<<m_node->GetId()<<std::endl;
...
Ptr<LiFiUserDeviceInfos> LiFiNetDevice::SearchForReceiverModel (Address address)
{
  ...
  //  std::cout<<"Ptr<LiFiUserDeviceInfos> LiFiTxNetDevice::SearchForReceiverModel (Address address) mac "<<mac<<std::endl;
		if (address == mac->GetAddress())
			return receiver;
...
{
 // if (m_node->GetId() == 8)
   // std::cout<<"LiFiNetDevice::Receive (Ptr<Packet> packet, uint16_t protocol, Mac48Address to, Mac48Address from) "<<m_node->GetId()<<std::endl;
