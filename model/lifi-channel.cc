...
{
  //std::cout<<"LiFiChannel::Send (Ptr<const Packet> p, Ptr<LiFiPhy> sender) "<<std::endl;
  ...
  for (std::vector<Ptr<LiFiPhy> >::const_iterator i = m_PhyVector.begin (); i != m_PhyVector.end (); ++i)
    ...
    if (rxDevice->GetDeviceType() =="UD" && sender->GetDeviceType() =="UD")
    {
     // std::cout<<"sender and rx are uds"<<std::endl;
     continue;
    }
    ...
    else
    {
     ...
     // std::cout<<"        Simulator::ScheduleWithContext (rxDevice->GetDevice()->GetNode ()->GetId ();"<<rxDevice->GetDevice()->GetNode ()->GetId ()<<std::endl;
        Simulator::ScheduleWithContext (rxDevice->GetDevice()->GetNode ()->GetId (),(propagationTime), &LiFiPhy::Receive,rxDevice,p->Copy (),sender->GetDevice()->GetNode()->GetObject<MobilityModel>());
    //}
