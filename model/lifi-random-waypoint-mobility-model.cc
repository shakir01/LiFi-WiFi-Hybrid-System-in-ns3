  ...
  static TypeId tid = TypeId ("ns3::LiFiRandomWaypointMobilityModel")
    ...
    .AddConstructor<LiFiRandomWaypointMobilityModel> ()
     .AddAttribute ("Speed",
                   ...
                   StringValue ("ns3::UniformRandomVariable[Min=0.7|Max=1.2]"),
...
void LiFiRandomWaypointMobilityModel::InitiateWalk (void)
{
  ...
  if (!start)
  {
     m_event             = Simulator::Schedule (Time(Seconds(3.0)), &LiFiRandomWaypointMobilityModel::BeginWalk,this,m_current,m_current,destination,m_currentmov,walkAngle,29.7);
...
ns3::RWPData* LiFiRandomWaypointMobilityModel::GetMobilityData()
{
  if (m_RWPDataVector.size() > 0)
  {
    ...
    //  std::cout<<"ns3::RWPData* LiFiRandomWaypointMobilityModel::GetMobilityData() "<<m_RWPDataVector.size()<<std::endl;
