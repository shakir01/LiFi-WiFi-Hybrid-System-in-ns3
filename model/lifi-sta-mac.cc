...
void LiFiStaMac::SendAssociationRequest ()
{
  ...
  //std::cout<<"m_MacLow "<<GetMacLow()<<std::endl;
  responseTag.SetTrafficType(GetDevice()->GetTrafficType());
  Ptr<LiFiPhy> lifiPhy = GetMacLow()->GetPhy();
  std::vector <ChannelStats > chStatsVect = lifiPhy->GetChannelData();
  ChannelStats channelStats = chStatsVect[chStatsVect.size()-1];
  //std::cout<<"chStatsVect size "<<chStatsVect.size()<<" Mac Low: "<<GetMacLow()<<" SNR at STA: "<<10*log(channelStats.snr)<<" Error: "<<channelStats.errRate<<std::endl;

  responseTag.SetSnrLevel(channelStats.snr);
  responseTag.SetError(channelStats.errRate);
  responseTag.SetPktDrop(0);
