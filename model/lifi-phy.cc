...
void LiFiPhy::SetConcentrationGain() 
{
	...
	//std::cout<<"this->m_concentrationGain "<<this->m_concentrationGain<<" m_refIndex "<<m_refIndex<<" m_FOVangle "<<m_FOVangle<<std::endl;
}
...
//std::cout<<"pyh layer snr "<<10*log(snr)<<" loss "<<loss<<std::endl;
	...
	if (snr > 100)
	{
		//std::cout<<"Sim Time: "<<Simulator::Now().GetSeconds()<<" Node ID: "<<this->GetDevice()->GetNode()->GetId()<<" snr "<<snr<<" loss "<<loss<<" snr1 "<<snr1<<std::endl;
	...
	//std::cout<<"Receive (Ptr<Packet> packet) error model "<<GetErrorModel ()<<" error "<<error<<"  snr "<<snr<<" time "<<ns3::Simulator::Now().GetSeconds()<<std::endl;
	if (m_error && m_error->IsCorrupt (packet))
	{
	
		//std::cout<<"Packet dropped "<<m_deviceType<<" Time: "<<ns3::Simulator::Now().GetSeconds()<<std::endl;
		return;
	}
...
std::vector <ChannelStats > LiFiPhy::GetChannelData()
{
	return channelStatsVector;
}
